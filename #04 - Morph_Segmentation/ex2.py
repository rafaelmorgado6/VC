...
diameter = 11
kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (diameter, 1))
kernel_sqr = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))


# Aplicar a dilatação na imagem negativa
eroded_image = cv2.erode(inverted_image, kernel)
eroded_sqr_image = cv2.erode(inverted_image, kernel_sqr, anchor=(0, 1))
eroded_sqr_image1 = cv2.erode(eroded_sqr_image, kernel_sqr, anchor=(0, 1))
eroded_sqr_image2 = cv2.erode(eroded_sqr_image1, kernel_sqr, anchor=(0, 1))
...
axes[1, 1].imshow(eroded_sqr_image, cmap="gray")
axes[1, 1].set_title("Eroded Square Image")
axes[1, 2].imshow(eroded_sqr_image1, cmap="gray")
axes[1, 2].set_title("Eroded Square Image 1")
axes[2, 0].imshow(eroded_sqr_image2, cmap="gray")
axes[2, 0].set_title("Eroded Square Image 2")
